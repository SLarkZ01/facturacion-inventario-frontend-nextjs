...
function Input({ className, type, ...props }: React.ComponentProps<"input">) {
  // Extract value from props so we can normalize it and avoid accidentally
  // letting a later spread of `props` overwrite the safe value. This ensures
  // the input never switches between uncontrolled/controlled.
  const { value, ...restProps } = props as any;
  const hasValueProp = Object.prototype.hasOwnProperty.call(props, "value");
  const safeValue = hasValueProp ? (value ?? "") : undefined;
